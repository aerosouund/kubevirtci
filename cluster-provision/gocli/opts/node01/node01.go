...
func (n *Node01Provisioner) Exec() error {
	...
	cmds := []string{
		...
		`[ -f /home/vagrant/enable_audit ] && echo "$(head -1 /etc/kubernetes/audit/adv-audit.yaml)" > /etc/kubernetes/audit/adv-audit.yaml && echo '` + string(advAudit) + `' | sudo tee /etc/kubernetes/audit/adv-audit.yaml > /dev/null`,
