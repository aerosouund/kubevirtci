...
func (n *Node01Provisioner) Exec() error {
	cmds := []string{
		...
		`[ -f /sys/fs/cgroup/cgroup.controllers ] && echo "Configuring cgroup v2"  && sudo mkdir -p /etc/crio/crio.conf.d && sudo echo '[crio.runtime]\nconmon_cgroup = "pod"\ncgroup_manager = "cgroupfs"' > /etc/crio/crio.conf.d/00-cgroupv2.conf && sudo sed -i 's/--cgroup-driver=systemd/--cgroup-driver=cgroupfs/' /etc/sysconfig/kubelet && sudo systemctl stop kubelet && sudo systemctl restart crio && sudo systemctl start kubelet`,
		...
		`sudo kubectl --kubeconfig=/etc/kubernetes/admin.conf create -f /provision/local-volume.yaml`,
