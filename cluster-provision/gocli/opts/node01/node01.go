...
func (n *Node01Provisioner) Exec() error {
	cmds := []string{
		...
		`sudo su; [ -f /sys/fs/cgroup/cgroup.controllers ] && echo "Configuring cgroup v2" && CRIO_CONF_DIR=/etc/crio/crio.conf.d && mkdir -p ${CRIO_CONF_DIR} && echo '[crio.runtime]\nconmon_cgroup = "pod"\ncgroup_manager = "cgroupfs"' > ${CRIO_CONF_DIR}/00-cgroupv2.conf && sed -i 's/--cgroup-driver=systemd/--cgroup-driver=cgroupfs/' /etc/sysconfig/kubelet && systemctl stop kubelet && systemctl restart crio && systemctl start kubelet`,
		"while [[ $(systemctl status crio | grep -c active) -eq 0 ]]; do sleep 2; done",
		"sudo swapoff -a",
		"until ip address show dev eth0 | grep global | grep inet6; do sleep 1; done",
		`kubeadm init --config "$kubeadm_conf" -v5`,
		`kubectl --kubeconfig=/etc/kubernetes/admin.conf patch deployment coredns -n kube-system -p "$(cat /provision/kubeadm-patches/add-security-context-deployment-patch.yaml)"`, // todo: dont make this depend on the node container
