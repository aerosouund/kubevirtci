...
func run(cmd *cobra.Command, args []string) (retErr error) {
	...
	for x := 0; x < int(nodes); x++ {
		...
		for _, cmd := range []string{"sudo mkdir /scripts", "sudo cp -r /home/vagrant/scripts/* /scripts"} {
			if _, err = sshClient.SSH(cmd, false); err != nil {
				return err
			}
		}
		...
		sshClient, err = sshutils.NewSSHClient(sshPort, x+1, true)
...
func provisionK8sOptions(sshClient sshutils.SSHClient, n *nodesconfig.NodeK8sConfig, k8sClient k8s.K8sDynamicClient) error {
	if n.Ceph {
...
func provisionNode(sshClient sshutils.SSHClient, n *nodesconfig.NodeLinuxConfig) error {
	...
	if n.SingleStack {
		if _, err := sshClient.SSH("touch /home/vagrant/single_stack", false); err != nil {
			return fmt.Errorf("provisioning node %d failed (setting singleStack phase): %s", n.NodeIdx, err)
		}
	}

	if n.EnableAudit {
		if _, err := sshClient.SSH("touch /home/vagrant/enable_audit", false); err != nil {
